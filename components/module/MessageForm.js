...
export default function MessageForm() {
    ...
    const validationData = (data) => {
        ...
        if (key === 'message') {
            ...
            (value.length < 4 || value.length > 20) ? setUserNameValid(false) : setUserNameValid(true)
        } else if (key === "email") {
    ...
    return (
        <div className='max-w-4xl mt-20 px-6 text-center md:px-12 text-2xl md:text-4xl m-auto'>
            <h3>Contact Us</h3>
            <div className="flex bg-slate-200  dark:bg-gray-400 m-1 my-6 mx-4 overflow-hidden rounded-2xl">
            ...
            <button onClick={sendMessageHandler} disabled={!userNameValid || !emailValid || !messageValid} className='py-2 px-6 md:py-3 md:px-16 rounded-xl  text-sm text-white bg-blue-600'>Send</button>
        </div>
    )
}
